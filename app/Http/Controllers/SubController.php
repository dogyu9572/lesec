...
//프로그램
    ...
    public function middle_semester_apply_a2(Request $request)
    {
        $gNum = "01"; $sNum = "01"; $gName = "프로그램"; $sName = "중등학기";
        
        // 현재 년월 (요청 파라미터로 받거나 현재 날짜)
        ...
        $programs = ProgramReservation::query()
            ->byEducationType('middle_semester')
            ->byApplicationType('group')
            ->active()
            ->whereBetween('education_start_date', [$startDate, $endDate])
        ...
        foreach ($programs as $program) {
            $date = $program->education_start_date->format('Y-m-d');
            if (!isset($programsByDate[$date])) {
                $programsByDate[$date] = [];
            }
            $programsByDate[$date][] = $program;
        }
        
        // 캘린더 구조 생성
        $calendar = $this->generateCalendar($year, $month, $programsByDate);
        
        return view('program.middle_semester_apply_a2', compact('gNum', 'sNum', 'gName', 'sName', 'programs', 'programsByDate', 'calendar', 'year', 'month'));
    }
    
    /**
     * 캘린더 구조 생성
     */
    private function generateCalendar($year, $month, $programsByDate)
    {
	...
	//중등학기 - 신청 - 단체 - 교육 선택 AJAX (달력 데이터)
    public function getGroupProgramsByMonth(Request $request)
    {
        $educationType = $request->input('education_type', 'middle_semester');
        $year = $request->input('year', now()->year);
        $month = $request->input('month', now()->month);
