...
// 이미지 업로드 함수 (운영/개발 동일 URL 사용, 에러 시 메시지 표시)
function uploadImage(file, editor) {
    const formData = new FormData();
    formData.append('image', file);
    formData.append('_token', window.csrfToken || document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'));

    var uploadUrl = (typeof document !== 'undefined' && document.body && document.body.dataset.uploadImageUrl)
    ...
    .then(function(response) {
        return response.text().then(function(text) {
            ...
            try {
                ...
                throw new Error(response.status + ' ' + (response.statusText || '서버 오류'));
            }
            if (!response.ok) {
    ...
    .catch(function(err) {
        ...
        alert('이미지 업로드 실패: ' + (err.message || '오류가 발생했습니다. 네트워크·권한·용량을 확인해주세요.'));
    });
}
