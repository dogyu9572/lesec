document.addEventListener("DOMContentLoaded", () => {
  ...
  const selectedMembersDisplay = document.getElementById(
    "selected_members_display"
  );
  const memberSelectionStatus = document.getElementById(
  ...
  let isSyncingRecipients = false;
  let syncDebounceTimer = null;

  function scheduleSyncRecipients() {
    if (!config.syncRecipientsUrl) {
      return;
    }
    ...
    syncDebounceTimer = setTimeout(() => {
      ...
      syncRecipientsToServer();
    }, 150);
  }

  function getSelectedMemberIdsFromDom() {
    return Array.from(
      selectedMembersBody.querySelectorAll('input[name="member_ids[]"]')
    )
      .map((input) => parseInt(input.value, 10))
      .filter((id) => Number.isInteger(id) && id > 0);
  }

  function syncRecipientsToServer() {
    if (!config.syncRecipientsUrl) {
      return;
    }

    if (isSyncingRecipients) {
      return;
    }

    const memberIds = getSelectedMemberIdsFromDom();
    if (!memberIds.length) {
      return;
    }

    isSyncingRecipients = true;
    setMemberSelectionStatus("수신자 목록을 저장하는 중입니다...", "info");

    const payload = {
      member_ids: memberIds,
      member_group_id: memberGroupSelect?.value
        ? parseInt(memberGroupSelect.value, 10)
        : null,
    };

    fetch(config.syncRecipientsUrl, {
      method: "POST",
      ...
      .then(async (response) => {
        ...
        if (!response.ok) {
          // Laravel가 419/500 등에서 HTML을 반환할 수 있어서 텍스트로 안전하게 처리
          const text = await response.text();
          const msg = `수신자 저장 실패 (${response.status})`;
          throw new Error(
        ...
        if (!contentType.includes("application/json")) {
          throw new Error("수신자 저장 실패 - 응답이 JSON이 아닙니다.");
        }
        return response.json();
      })
      .then((data) => {
        if (!data || !data.success) {
          const msg = (data && data.message) || "수신자 저장에 실패했습니다.";
          setMemberSelectionStatus(msg, "error");
          return;
        }

        setMemberSelectionStatus("수신자 목록이 저장되었습니다.", "success");

        if (
          typeof window.setRecipientCount === "function" &&
          typeof data.total === "number"
        ) {
          window.setRecipientCount(data.total);
        }

        // 수신자 목록이 열려있다면 즉시 갱신
        if (typeof window.loadRecipients === "function") {
          const recipientsContent =
            document.getElementById("recipientsContent");
          if (recipientsContent && recipientsContent.style.display !== "none") {
            window.loadRecipients(1);
          }
        }
      })
      .catch((error) => {
        console.error(error);
        setMemberSelectionStatus(
          `수신자 저장 중 오류: ${error?.message || "다시 시도해 주세요."}`,
          "error"
        );
      })
      .finally(() => {
        isSyncingRecipients = false;
      });
  }
  ...
  const existingMemberRows =
    selectedMembersBody.querySelectorAll("tr[data-member-id]");
  ...
  function openMemberSearchModal() {
    const url =
      "/backoffice/popup-windows/member-search?selection_mode=multiple";
    const width = window.innerWidth <= 768 ? "100%" : "1000";
    const height = window.innerHeight <= 768 ? "100%" : "700";
    window.open(
      url,
      "memberSearch",
      `width=${width},height=${height},left=100,top=100,scrollbars=yes,resizable=yes`
    );
  }
  ...
  window.applySelectedMember = function (selectedMember) {
    ...
    // 배열로 전달된 경우 (여러 명 선택)
    const selectedMembers = Array.isArray(selectedMember)
      ? selectedMember
      : [selectedMember];
  ...
  function clearModalSelection() {
    ...
    if (memberListBody) {
      const checkboxes = memberListBody.querySelectorAll(
        ".popup-member-checkbox"
      );
  ...
  function renderSelectedMembersEmptyRow() {
    ...
    const hasMemberRows =
      selectedMembersBody.querySelectorAll("tr[data-member-id]").length > 0;

    if (hasMemberRows || selectedMemberIds.size > 0) {
      const emptyRow = selectedMembersBody.querySelector(
  ...
  function updateSelectedMembersDisplay() {
    ...
    const names = Array.from(
      selectedMembersBody.querySelectorAll("tr[data-member-id] td:first-child")
    )
  ...
  // 선택 목록이 다른 코드/렌더링으로 변경되더라도 display/input이 항상 동기화되도록 감시
  const selectedMembersObserver = new MutationObserver(() => {
    updateSelectedMembersDisplay();
    scheduleSyncRecipients();
  });
  selectedMembersObserver.observe(selectedMembersBody, {
    childList: true,
    subtree: true,
  });
  ...
  function setMemberSelectionStatus(message, tone = "info") {
    ...
    memberSelectionStatus.classList.remove(
      "text-danger",
      "text-success",
      "text-muted"
    );
  ...
  function fetchGroupMembers(groupId) {
    ...
    fetch(url, {
      ...
      .then((data) => {
        ...
        const count =
          typeof meta.count === "number" ? meta.count : members.length;

        if (members.length === 0) {
          setMemberSelectionStatus(
            `${groupName}에 등록된 회원이 없습니다.`,
            "error"
          );
        } else {
          setMemberSelectionStatus(
            `${groupName} 회원 ${count}명이 자동으로 선택되었습니다.`,
            "success"
          );
        }
      ...
      .catch((error) => {
        ...
        setMemberSelectionStatus(
          "회원 그룹 정보를 불러오지 못했습니다. 다시 시도해 주세요.",
          "error"
        );
      });
  }
  ...
  function resetToManualSelection() {
    ...
    clearSelectedMembers();
    setMemberSelectionStatus(
      "회원 그룹을 선택하거나 검색 버튼으로 회원을 추가해 주세요.",
      "info"
    );
  }
  ...
  function renderMemberList(members) {
    ...
    memberListBody
      .querySelectorAll(".popup-member-checkbox")
      .forEach((checkbox) => {
  ...
  selectedMembersBody.addEventListener("click", (event) => {
    const removeBtn = event.target.closest(".selected-member-remove");
    if (!removeBtn) {
      return;
    }

    const memberId = parseInt(removeBtn.dataset.memberId, 10);
    if (!memberId) {
      return;
    }

    selectedMemberIds.delete(memberId);
    const row = selectedMembersBody.querySelector(
  ...
  function toggleConfirmButton() {
    ...
    if (memberListBody && confirmBtn) {
      const hasSelection =
        memberListBody.querySelectorAll(".popup-member-checkbox:checked")
          .length > 0;
  ...
  if (selectAllCheckbox && memberListBody) {
    selectAllCheckbox.addEventListener("change", (event) => {
      ...
      memberListBody
        .querySelectorAll(".popup-member-checkbox:not(:disabled)")
        .forEach((checkbox) => {
  ...
  // 편집 화면 진입 시, 화면의 선택 목록과 DB 수신자 목록이 불일치하는 경우가 있어 1회 동기화
  // (예: 팝업 선택 직후 저장되지 않았거나, 기존 값이 old()로만 남아있는 경우)
  if (config.syncRecipientsUrl && selectedMemberIds.size > 0) {
    syncRecipientsToServer();
  }
});
