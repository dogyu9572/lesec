...
/* 일정 정렬 및 연속 막대 처리 로직 */
document.addEventListener('DOMContentLoaded', function() {
    const dayCells = [...document.querySelectorAll('td.calendar-day')];
    
    // 1. 날짜 순으로 정렬
    dayCells.sort((a, b) => a.dataset.date.localeCompare(b.dataset.date));

    let appIdCounter = 1;
    const appIdMap = new Map(); // "제목|정원" -> 고유 ID
    const scheduleSpans = new Map(); // 고유 ID -> { 시작날짜, 종료날짜, 차지하는 칸수 }

    // 2. 모든 일정에 고유 appid 부여 및 스팬(기간) 계산
    dayCells.forEach(td => {
        const items = td.querySelectorAll('.list li:not(.dummy)');
        items.forEach(li => {
            const key = [li.getAttribute('title') || '', li.dataset.total || ''].join('|');
            if (!appIdMap.has(key)) {
    ...
    // 3. 주 단위(tr) 또는 전체 범위에서 globalOrder와 막대 너비 계산
    // 여기서는 주별(tr)로 처리하거나 전체를 하나로 처리할 수 있는데, 
    // 제시하신 결과값처럼 1~7일(한 줄) 기준으로 처리합니다.
    const rows = document.querySelectorAll('tr'); // 달력의 각 주(row)
    rows.forEach(row => {
        const cells = [...row.querySelectorAll('td.calendar-day')];
        if (cells.length === 0) return;

        let globalOrder = [];
        const appidData = {}; // 각 appid의 시작 위치와 길이를 저장

        // 해당 주의 모든 appid 순서 파악
        cells.forEach((td, colIdx) => {
            const items = td.querySelectorAll('.list li:not(.dummy)');
            items.forEach(li => {
                const appid = li.dataset.appid;
                if (!globalOrder.includes(appid)) {
                    globalOrder.push(appid);
                    appidData[appid] = { start: colIdx, count: 1 };
                } else {
                    appidData[appid].count++;
                }
            });
        });
        ...
        // 4. DOM 재배치 및 스타일링
        cells.forEach((td, colIdx) => {
            const list = td.querySelector('.list');
            if (!list) return;

            const originalItems = new Map();
            list.querySelectorAll('li:not(.dummy)').forEach(li => {
                originalItems.set(li.dataset.appid, li);
            });

            list.innerHTML = ''; // 초기화

            globalOrder.forEach(appid => {
                const data = appidData[appid];
                
                if (originalItems.has(appid)) {
                    const li = originalItems.get(appid);
                    
                    if (data.start === colIdx) {
                        // 시작점: 너비 계산 (예: calc(300% + 18px))
                        if (data.count > 1) {
                            // 9px는 td 사이의 간격(border-spacing/margin)에 따라 조정 필요
                            li.style.width = `calc(${data.count * 100}% + ${(data.count - 1) * 9}px)`;
                            li.style.zIndex = "10";
                        }
                    } else {
                        // 연속된 부분: 글자 숨기기(off 클래스)
                        li.classList.add('off');
                    }
                    list.appendChild(li);
                } else {
                    // 해당 위치에 일정이 없으면 dummy 생성
