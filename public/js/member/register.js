(function ($) {
  ...
  function MemberRegister(form) {
    ...
    this.selectedSchoolId =
      (this.$form.find(".input_school_id").val() || "").trim() || null;
    this.schoolDataLoaded = false;
    this.serverErrors = this.$form.data("errors") || null;
    this.serverErrorKeys = this.serverErrors
  ...
  MemberRegister.prototype.bindDuplicateCheck = function () {
    ...
    this.$form.on("click", ".js-duplicate-check", function (event) {
      ...
      if ($button.data("email")) {
        ...
        var domainValue = $emailDomain.length
          ? ($emailDomain.val() || "").trim()
          : "";
      ...
      function showResult(success, message) {
        ...
        if (success) {
          ...
          $wrapper.append(
            '<p class="success_alert c_green">' + message + "</p>"
          );
  ...
  MemberRegister.prototype.bindInputChange = function () {
    ...
    this.$form.on(
      "input",
      '[name="contact"], [name="student_contact"]',
      function () {
        var $input = $(this);
  ...
  MemberRegister.prototype.bindPhoneFormatting = function () {
    ...
    $inputs.each(function () {
      ...
      $input.on("input", function (e) {
        ...
        var digitsBefore = self.normalizePhone(
  ...
  MemberRegister.prototype.focusServerErrorField = function () {
    ...
    // 모든 에러를 표시하고 첫 번째 에러로만 스크롤
    // 에러 필드 우선순위: 약관 동의 > 필수 입력 필드 > 기타
    var priorityFields = [
      "privacy_agree",
      "notification_agree",
      "login_id",
    ...
    // 우선순위 필드 먼저 추가
    priorityFields.forEach(
      function (field) {
        if (this.serverErrorKeys.indexOf(field) !== -1) {
          sortedErrorKeys.push(field);
        }
      }.bind(this)
    );

    // 나머지 필드 추가
    this.serverErrorKeys.forEach(function (field) {
      if (sortedErrorKeys.indexOf(field) === -1) {
        sortedErrorKeys.push(field);
      }
    });

    // 첫 번째 에러로 스크롤
    if (sortedErrorKeys.length > 0) {
      var selector = this.getErrorFieldSelector(sortedErrorKeys[0]);
      if (selector) {
  ...
  MemberRegister.prototype.restoreDuplicateCheckStatus = function () {
    ...
    if (loginIdVerified === "1") {
      ...
      if (loginIdValue && loginIdValue.trim() !== "") {
        ...
        if (!$loginIdWrapper.find(".success_alert").length) {
          $loginIdWrapper.append(
            '<p class="success_alert c_green">사용 가능한 아이디 입니다.</p>'
          );
        }
      }
    }
    ...
    if (contactVerified === "1") {
      var $contactWrapper = this.$form
        .find('[name="student_contact"]')
        .closest("dd");
      ...
      if (contactValue && contactValue.trim() !== "") {
        ...
        if (!$contactWrapper.find(".success_alert").length) {
          $contactWrapper.append(
            '<p class="success_alert c_green">사용 가능한 연락처 입니다.</p>'
          );
        }
      }
    }
    ...
    $.ajax({
      ...
      .done(function (data) {
        ...
        self.updateResultMessage(
          data && data.meta ? data.meta.count : list.length
        );
  ...
  MemberRegister.prototype.renderSchools = function (list) {
    ...
    if (!list.length) {
      $tbody.append(
        '<tr class="empty"><td colspan="3">검색 결과가 없습니다.</td></tr>'
      );
    ...
    list.forEach(function (school) {
      ...
      var rowHtml =
        ...
        (school.name || "") +
        "</strong></td>" +
        '<td><label class="check solo"><input type="radio" name="selected_school" value="' +
        school.id +
        '"' +
        (isSelected ? " checked" : "") +
        "><i></i></label></td>" +
  ...
  MemberRegister.prototype.setSelectOptions = function (
    $select,
    items,
    placeholder
  ) {
  ...
  MemberRegister.prototype.updateResultMessage = function (
    count,
    selectedName
  ) {
  ...
  MemberRegister.prototype.highlightSelectedSchool = function () {
    ...
    this.$modal
      .find(
        'input[name="selected_school"][value="' + this.selectedSchoolId + '"]'
      )
      .prop("checked", true)
      .trigger("change");
  };
  ...
  MemberRegister.prototype.applySchool = function (school) {
    ...
    if ($districtSelect.length && school.district) {
      if (
        !$districtSelect.find('option[value="' + school.district + '"]').length
      ) {
  ...
  MemberRegister.prototype.bindPasswordPreservation = function () {
    ...
    var $passwordTemp = this.$form.find(".password-temp");
    var $passwordConfirmTemp = this.$form.find(".password-confirmation-temp");

    // 비밀번호 입력 시 hidden 필드에 저장
    if ($passwordInput.length) {
      $passwordInput.on("input", function () {
        if ($passwordTemp.length) {
          $passwordTemp.val($(this).val());
        }
      });
    }

    // 비밀번호 확인 입력 시 hidden 필드에 저장
    if ($passwordConfirmInput.length) {
      $passwordConfirmInput.on("input", function () {
        if ($passwordConfirmTemp.length) {
          $passwordConfirmTemp.val($(this).val());
        }
      });
    }
    ...
    this.$form.on("submit", function () {
      if ($passwordInput.length && $passwordTemp.length) {
        $passwordTemp.val($passwordInput.val());
      }
      if ($passwordConfirmInput.length && $passwordConfirmTemp.length) {
        $passwordConfirmTemp.val($passwordConfirmInput.val());
      }
    });
  };

  MemberRegister.prototype.restorePasswordValues = function () {
    var $passwordInput = this.$form.find('[name="password"]');
    var $passwordConfirmInput = this.$form.find(
      '[name="password_confirmation"]'
    );
    ...
    // hidden 필드에 값이 있으면 항상 복원 (old() 값보다 우선)
    if ($passwordInput.length && $passwordTemp.length) {
      var tempValue = $passwordTemp.val();
      if (tempValue) {
        $passwordInput.val(tempValue);
      } else if ($passwordInput.val()) {
        // 입력 필드에 값이 있으면 hidden 필드에 저장
        $passwordTemp.val($passwordInput.val());
      }
    }

    if ($passwordConfirmInput.length && $passwordConfirmTemp.length) {
      var tempConfirmValue = $passwordConfirmTemp.val();
      if (tempConfirmValue) {
        $passwordConfirmInput.val(tempConfirmValue);
